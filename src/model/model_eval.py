...
def main():
    try:
        ...
        with mlflow.start_run() as run:
            ...
            model_dir = "Best_Model_mlflow"
            if os.path.exists(model_dir):
                shutil.rmtree(model_dir)
            # Save the model locally in MLflow format (no remote registry API calls)
            mlflow.sklearn.save_model(
                sk_model=model,
                path=model_dir,
                signature=signature
            )
            # Upload the saved MLflow model folder as artifacts (DagsHub-safe)
